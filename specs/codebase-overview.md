...
| `ai_agent.py` | `SupportedModels` 枚举（`openai:gpt-4o`、`deepseek:deepseek-chat`）；`DivinationAgent` 用 pydantic-ai `Agent` 封装。`get_available_models()` 靠 `.env` 里有无对应 API key 判断可用性。system prompt 要求输出 `###` 标题 + `**粗体**` + 列表，≤1000 字 | `hand_technique.py`、两个 UI（模型选择） |
