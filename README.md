...
#### Web版本
- 暖白纸感与朱砂配色，桌面左右分栏、手机纵向阅读
- 首屏即见起课输入与「开始占卜」按钮；问题与 AI 解读为可折叠选填区，配置 AI 时默认展开
- 起课前展示三传阶段引导，内置可随时打开的使用指南
- 数字、日期、汉字三种输入，支持离线计算
- 宽屏三列、窄屏纵向三传卡片，完整显示释义
- 本次输入与起课数字快照，五行关系明确标注方向
- 传统方位与神灵信息按需展开
- 可访问输入标签、等待/完成/失败状态，失败后保留本地结果
- 四套皮肤：素纸（默认）、星夜、水墨、霓虹；头部一键切换，浏览器记忆选择，支持 `?theme=night` 等参数
...
2. **开始占卜**：点击按钮，右侧立即呈现本地三传；核对起课数字，阅读初传、两段关系和后续趋势，文化背景可展开。
3. **心中所问 · 解读方式（选填）**：展开选填区填写具体问题并选择可用 AI 模型，再次点击「开始占卜」获得针对性解读。
4. **查看AI解读**：一句话判断 → 初传（起点）→ 中传（过程）→ 末传（趋势）→ 行动建议。每传都有依据、白话与建议。
5. **切换皮肤（可选）**：头部色样按钮在素纸、星夜、水墨、霓虹之间切换，仅改变外观与动效，不影响计算与解读。

| 皮肤 | 视觉 | 交互 |
|---|---|---|
| 素纸（默认） | 暖白纸感 · 朱砂主色 | 现有体验 |
| 星夜 | 深空星图 · 鎏金流光，五行分色徽章 | 星空闪烁与流星、随指针移动的提灯光晕、三传交错浮现 |
| 水墨 | 宣纸留白 · 桌面竖排标题、笔触分隔、朱文印章 | 印章盖印入场、三传墨晕显现、悬停划出墨线 |
| 霓虹 | 赛博终端 · 网格、扫描线、HUD 角标 | 切换时 CRT 开机、标题悬停故障字效、卡片扫描光带 |

皮肤选择保存在浏览器 `localStorage`，URL 参数 `?theme=<id>` 可指定并记忆；遵守系统“减少动态效果”偏好。
...
│   ├── web.css             # Web视觉样式与响应式布局（默认皮肤）
│   ├── themes/             # 皮肤：theme.js 切换运行时，night/ink/neon.css
